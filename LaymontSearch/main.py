...
bot = Bot(token="6089866020:AAFTyH-2-ehFGMlv3adeijjN4D5a4MdOjzk")

dp = Dispatcher(bot)

print('ЛПЗВАМЛОЩДВПОДЛВАПРОЛЖДВАЛОПВАЛОДЖПВОДАДЛВАОПДОЛВАПЛЗДВАПОЛВА')
    ...
    if len(channels_me) == 0:
        ...
        for i in channels_me:
            ...
            if user_channel_status["status"] == 'left':
                ...
                inline_btn_2 = InlineKeyboardButton('Готово ✅', callback_data='check')
...
async def vote_up_cb_handler(call):
    ...
    if c == 1:
       ...
       await bot.send_message(call.from_user.id, text = 'Привет это *Laymont Search | Поиск Кино 🍿*\nЧтобы узнать название фильмов из ЮТУБ | ТИК ТОК вам нужно ввести код фильма.\nНажмите на *«🔎Поиск» 👇*',parse_mode = 'Markdown',reply_markup = start.greet_kb1)
